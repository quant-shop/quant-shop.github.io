---
title: "Data Wells: Race and State Violence in the United States from 1892"
subtitle: "Quantitative Histories Workshop"
author:
  - name: Nathan Alexander
    affiliations:
      - ref: hu
  - name: Kade Davis
    affiliations:
      - ref: mc
  - name: Basil Ghali
    affiliations:
      - ref: hu
  - name: Qyana Stewart
    affiliations:
      - ref: hu
  - name: Gabriella La Cour
    affiliations:
      - ref: sc
affiliations:
  - id: hu
    name: Howard University
    city: Washington
    state: DC
  - id: mc
    name: Morehouse College
    city: Atlanta
    state: GA
  - id: sc
    name: Spelman College
    city: Atlanta
    state: GA
  - id: cd
    name: Center for Applied Data Science and Analytics
    city: Washington
    state: DC
format:
  pdf:
    pdf:
    toc: true
    number-sections: true
    colorlinks: true
    self-contained: true
    embed-resources: true 
  html:
    self-contained: true
    embed-resources: true
  revealjs:
    output-file: 2024_04_27_bob_moses.qmd
    height: 900
    width: 1600
    smaller: false
    scrollable: true
    slide-number: c/t #< collapsted/total
    footer: ""
    logo: "howard-logo.png"
    toc: false
    echo: true
    incremental: false
    self-contained: true
    theme: default
---

## [*Data*]{style="color:darkgreen;"} + Ida B. [*Wells*]{style="color:darkgreen;"}-Barnett

::: {.incremental}

  - We use the term "[Data Wells]{style="color:darkgreen;"}" to describe how we practice the identification, input, and storage of what can be termed as critical insights data, or CIDs.
  
  - We use information in databases in four ways: 
  
    (1) studying problems in the [quantification of historical information]{style="color:red;"} across various axes: time, social constructs, and/or systemic issues, 
    
    (2) data [identification]{style="color:red;"} and [wrangling]{style="color:red;"}, 
    
    (3) data [analysis]{style="color:red;"} and [communication]{style="color:red;"}, and 
    
    (4) [modeling]{style="color:red;"} abstract inquiries.
  
  - We begin our analysis with Ida B. Wells-Barnett's organization and analysis of lynching.
  
  - We describe "Data Wells" of U.S. state violence using quantitative history as a frame.

:::

# Quantitative Histories Workshop {background-color="lightgrey"}

`Curriculum & software development collective` 

and

[research lab]{style="color:orange;"}

```{r}
#| echo: false
#| output: false
#| warning: false

# install all required packages
require("terra")
require(tidyverse)
install.packages("tidycensus", repos = "http://cran.us.r-project.org")
require(tmap)
require("ggiraph")
require(mapdeck)
require(ggplot2)

# load all required libraries
library(tidycensus)
library(tidyverse)
library(tigris)
library(tidycensus)
library(tidyverse)
library(patchwork)
library(scales)
library(ggplot2)
library(sf)
library(patchwork)
install.packages("geojsonio", repos = "http://cran.us.r-project.org")
library(geojsonio)
```

## Quantitative history

:::: {.columns}

::: {.column width="40%"}

  - Quantitative history considers methods and approaches to artifacts as data and information.
  
  - Historians like Pierre Chanu (book to the right) are centered in traditional texts; more traditional perspectives are uncovering troubling practices with regard to race[^chanu].
  
  - Despite many long-standing critiques, there are [fewer critical dimensions]{style="color:orange;"} in [quantitative history] narratives.
  
:::

[^chanu]: Vovelle, Michel (1987). Bourgeoisies de province et Revolution. Presses Universitaires de Grenoble.

::: {.column width="60%"}

![[*Histoire Quantitative*](https://www.jstor.org/stable/1876930){target="blank"} by Pierre Chanu](quanthist.jpg){width=40%}

:::

::::

## Racialization and U.S. State Violence

Today, we will explore three forms of state sponsored violence.

. . .

  - Lynching 

![Ida B. Wells-Barnett](ida1.jpg){width=50%}

. . .

  - Police

. . .

  - Prisons

## The Red Record

  - **Personal experience**. In 1892, a close friend of Ida B. Wells-Barnett was lynched. Wells-Barnett, a known activist, community organizer, and journalist, would generate quantitative indicators of lynching as state violence.

. . .

  - **Intuition and method**. Like many Black communities at the time and other allies, Wells-Barnett acknowledged both the personal (micro) social forces of racism and the systemic (macro) nature of white racial violence. In this case, this violence was expressed through the practice of lynching.

. . .

  - **Impact**. Wells-Barnett's databases, and the use of number and quantification have a profound impact on the current view of state-sponsored racial violence.

## Lynching

Caitlin Pollock has created [software](https://redrecord.cmjpollock.com/) based on a series of extracted data from Wells-Barnett's work. Although the data provides for quick loading and analysis, it does require some data wrangling.

::: {.panel-tabset}

### 1893

Content for 1893

```{r}
#| echo: false
#| output: false
#| warning: false
library(geojsonio)
df1893 <- geojson_read("https://raw.githubusercontent.com/professornaite/red-record/main/data/1893map.geojson")
df1893
```

```{r}
#| echo: false
#| output: true
#| warning: false
library(geojsonio)

```


### 1893 map

```{r}
#| echo: false
#| output: false
#| warning: false
# 1893 map
library(dplyr)
library(ggplot2)
library(stringr)
library(magrittr)
install.packages("xts", repos = "http://cran.us.r-project.org")          # To make the convertion data-frame / xts format
library(xts)
install.packages("dygraphs", repos = "http://cran.us.r-project.org")
library(dygraphs)

## load date; cite from caitlin p (2015)
record1893 <- read.csv("/Users/nathanalexander/Dropbox/Projects/lynching/IdaBWellsProject/RedRecord/redrecord1893.csv")
head(record1893)
tail(record1893)
str(record1893)

# changes to data
record1893 %>% 
  rename(lat = Latitude,
         lon = Longitude) -> record1893

record1893$count <- str_count(record1893$Names, ",")+1 
head(record1893)

library(lubridate)
install.packages("parsedate", repos = "http://cran.us.r-project.org")
library(parsedate)

# An example using parse_date() on the column date_onset
record1893 %>% 
  mutate(date = as.Date(Date, format = "%b.%d,%Y")) -> record1893

str(record1893)
glimpse(record1893)

ggplot(record1893, aes(x=date, y=count)) +
  geom_line( color="steelblue") + 
  geom_point() +
  xlab("Date") + 
  ylim(1,7)


don <- xts(x = record1893$count, order.by = record1893$date)
p <- dygraph(don) %>%
  dyOptions(labelsUTC = TRUE, fillGraph=TRUE, fillAlpha=0.1, drawGrid = FALSE, colors="#D8AE5A") %>%
  dyRangeSelector() %>%
  dyCrosshair(direction = "vertical") %>%
  dyHighlight(highlightCircleSize = 5, highlightSeriesBackgroundAlpha = 0.2, hideOnMouseOut = FALSE)  %>%
  dyRoller(rollPeriod = 1)
```


```{r}
#| echo: false
#| output: true
#| warning: false
p

```


### 1894

Content for 1894

```{r}
#| echo: false
#| output: true
#| warning: false
df1894 <- read_sf("https://raw.githubusercontent.com/quant-shop/IdaBWellsProject/master/RedRecord/redrecord1894.geojson.txt")
df1894

```

### 1895

Content for 1895

```{r}
#| echo: false
#| output: false
#| warning: false
install.packages("xml2", repos = "http://cran.us.r-project.org")
install.packages("XML", repos = "http://cran.us.r-project.org")
library(xml2)
library(XML)
df1895t <- read_xml("https://raw.githubusercontent.com/quant-shop/IdaBWellsProject/master/RedRecord/redrecord1895.xml")
df1895t
df1895s <- xmlParse(df1895t)
```

```{r}
#| echo: false
#| output: true
#| warning: false
df1895s
```

:::

---

## Concerns

Pollock deals with the issue of erasure in their development of the data.

## Policing {background-image="https://media.cnn.com/api/v1/images/stellar/prod/200709194059-09-week-in-photos-0710.jpg?q=w_1480,c_fill" background-opacity=0.5}


## Campus Policing {background-image="https://media.cnn.com/api/v1/images/stellar/prod/240425164732-30-university-campus-protests-unf.jpg?q=w_1480,c_fill" background-opacity=0.5}


## Prisons {background-image="https://www.urban.org/sites/default/files/shutterstock_365427041_crop.jpg" background-opacity=0.5}

::: {.incremental}

  - In 2021, Black Americans were imprisoned at 5.0 times the rate of whites, while American Indians and Latinx people were imprisoned at 4.2 times and 2.4 times the white rate, respectively ([The Sentencing Project](https://www.sentencingproject.org/reports/one-in-five-ending-racial-inequity-in-incarceration/), 2023)

  - One in five Black men born in 2001 is likely to experience imprisonment within their lifetime, a decline from one in three for those born in 1981. Pushback from policymakers threatens further progress in reducing racial inequity in incarceration.

:::

```{r}

```

```{r}

```


## Prisons

::: {.panel-tabset}

### Federal

Content for federal

### State

Content for state

### Federal and State

Content for federal and state

:::


# Thank you
. . .

Thank you for joining us and citing today's presentation.

Alexander, N., Davis, K., Ghali, B., Stewart, K., & La Cour, G. (2024, April 26). Data Wells: Race and State Violence in the United States from 1892. *The 2024 Bob Moses Conference*. [Online](https://www.bobmosesconference.com/){target="blank"}.

